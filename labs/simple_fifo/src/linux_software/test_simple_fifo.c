...
void read_fifo(volatile unsigned int *ptrToFifo, int* fifo_data, int num_reads) {
    for(int i=0; i<num_reads; i++){
        //Check to see if fifo is empty
        if(*(ptrToFifo+SIMPLE_FIFO_COUNTER_OFFSET) > 0){
            *(fifo_data + i) = *(ptrToFifo+SIMPLE_FIFO_DATA_OFFSET);
        }
    }
    check_fifo(fifo_data, num_reads);
}
...
void print_benchmark(float time, int num_reads){
    ...
    float throughput=0; 
    unsigned int bytes_transferred = 4*num_reads;				// # of Reads * 4bytes (32bits)
    throughput = bytes_transferred/time/1000000;				// bytes / time / 1MB
...
int main(int argc, char* argv[]) {
    ...
    // Menu
    printf("\r\n\r\n\r\n");
    printf("Milestone 1 Hunter Mills - Simple FIFO\n\r");
